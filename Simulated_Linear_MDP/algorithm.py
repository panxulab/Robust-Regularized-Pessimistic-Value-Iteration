...
class PEVI(meta_algorithm):
    def __init__(self, A, beta, H, lam, dataset, Rho, theta, fail_state):
        super().__init__(A, beta, H, lam, dataset, Rho, theta, fail_state)
    ...
    def get_Q_func(self, phi, h, s_f=False):
        if s_f == True:
            ...
            Lambda_h_inverse = np.linalg.inv(self.Lambda[h])
            Q_h = np.min([(self.w[h] @ phi - self.beta * np.sqrt(phi @ Lambda_h_inverse @ phi)), self.H - h])
            return Q_h
...
class VA_DRPVI(meta_algorithm):
    def __init__(self, pre_agent, A, beta, H, lam, dataset, Rho, theta, fail_state):
        super().__init__(A, beta, H, lam, dataset, Rho, theta, fail_state)
